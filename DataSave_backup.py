...
class DataSave:
    ...
    def _generate_path(self, root_path):
        ...
        folders = ['calib', 'image_2', 'image_seg', 'image_stereoright', 'image_stereoleft', 'kitti_label',
                   'carla_label', 'velodyne']
    ...
    def save_training_files(self, data):
        ...
        for agent, dt in data["agents_data"].items():
            ...
            # camera_front_transform = config_to_trans(self.cfg["SENSOR_CONFIG"]["RGB_Front"]["TRANSFORM"])
            # camera_right_transform = config_to_trans(self.cfg["SENSOR_CONFIG"]["RGB_Right"]["TRANSFORM"])
            # camera_left_transform = config_to_trans(self.cfg["SENSOR_CONFIG"]["RGB_Left"]["TRANSFORM"])
            # camera_rear_transform = config_to_trans(self.cfg["SENSOR_CONFIG"]["RGB_Rear"]["TRANSFORM"])
            ...
            # print("kitti_label_fname :",kitti_label_fname)

            save_calibration_matrices(
                [camera_raw_transform, camera_seg_transform, camera_sr_transform, camera_sl_transform, lidar_transform],
                calib_filename, dt["intrinsic"])
            # save_calibration_matrices(
