...
Constructor which creates the backing fields for the properties in the class
        ...
        self._T_camera_radar_arbe: Optional[np.ndarray] = None
        self._T_camera_radar_ars548: Optional[np.ndarray] = None

        self._T_lidar_camera: Optional[np.ndarray] = None
        self._T_radar_arbe_camera: Optional[np.ndarray] = None
        self._T_radar_ars548_camera: Optional[np.ndarray] = None
        self._T_lidar_radar_arbe: Optional[np.ndarray] = None
        self._T_lidar_radar_ars548: Optional[np.ndarray] = None
        self._T_radar_arbe_lidar: Optional[np.ndarray] = None
        self._T_radar_ars548_lidar: Optional[np.ndarray] = None
...
Property which returns the homogeneous transform matrix from the radar frame, to the camera frame.
        ...
        """
        if self._T_camera_radar_arbe is not None:
            # When the data is already loaded.
            return self._T_camera_radar_arbe
        else:
            # Load data if it is not loaded yet.
            self._camera_projection_matrix, self._T_camera_radar_arbe = self.get_sensor_transforms('radar_arbe')
            return self._T_camera_radar_arbe

    @property
    def t_camera_radar_ars548(self):
        """
Property which returns the homogeneous transform matrix from the radar frame, to the camera frame.
        :return: Numpy array of the homogeneous transform matrix from the radar frame, to the camera frame.
        """
        if self._T_camera_radar_ars548 is not None:
            # When the data is already loaded.
            return self._T_camera_radar_ars548
        else:
            # Load data if it is not loaded yet.
            self._camera_projection_matrix, self._T_camera_radar_ars548 = self.get_sensor_transforms('radar_ars548')
            return self._T_camera_radar_ars548
...
Property which returns the homogeneous transform matrix from the camera frame, to the radar frame.
        ...
        """
        if self._T_radar_arbe_camera is not None:
            # When the data is already loaded.
            return self._T_radar_arbe_camera
        else:
            # Calculate data if it is calculated yet.
            self._T_radar_arbe_camera = np.linalg.inv(self.t_camera_radar_arbe)
            return self._T_radar_arbe_camera

    @property
    def t_radar_ars548_camera(self):
        """
Property which returns the homogeneous transform matrix from the camera frame, to the radar frame.
        :return: Numpy array of the homogeneous transform matrix from the camera frame, to the radar frame.
        """
        if self._T_radar_ars548_camera is not None:
            # When the data is already loaded.
            return self._T_radar_ars548_camera
        else:
            # Calculate data if it is calculated yet.
            self._T_radar_ars548_camera = np.linalg.inv(self.t_camera_radar_ars548)
            return self._T_radar_ars548_camera

    @property
    def t_lidar_radar_arbe(self):
        """
Property which returns the homogeneous transform matrix from the radar frame, to the lidar frame.
        ...
        """
        if self._T_lidar_radar_arbe is not None:
            # When the data is already loaded.
            return self._T_lidar_radar_arbe
        else:
            # Calculate data if it is calculated yet.
            self._T_lidar_radar_arbe = np.dot(self.t_lidar_camera, self.t_camera_radar_arbe)
            return self._T_lidar_radar_arbe

    @property
    def t_lidar_radar_ars548(self):
        """
Property which returns the homogeneous transform matrix from the radar frame, to the lidar frame.
        :return: Numpy array of the homogeneous transform matrix from the radar frame, to the lidar frame.
        """
        if self._T_lidar_radar_ars548 is not None:
            # When the data is already loaded.
            return self._T_lidar_radar_ars548
        else:
            # Calculate data if it is calculated yet.
            self._T_lidar_radar_ars548 = np.dot(self.t_lidar_camera, self.t_camera_radar_ars548)
            return self._T_lidar_radar_ars548

    @property
    def t_radar_arbe_lidar(self):
        """
Property which returns the homogeneous transform matrix from the lidar frame, to the radar frame.
        ...
        """
        if self._T_radar_arbe_lidar is not None:
            # When the data is already loaded.
            return self._T_radar_arbe_lidar
        else:
            # Calculate data if it is calculated yet.
            self._T_radar_arbe_lidar = np.dot(self.t_radar_arbe_camera, self.t_camera_lidar)
            return self._T_radar_arbe_lidar

    @property
    def t_radar_ars548_lidar(self):
        """
Property which returns the homogeneous transform matrix from the lidar frame, to the radar frame.
        :return: Numpy array of the homogeneous transform matrix from the lidar frame, to the radar frame.
        """
        if self._T_radar_ars548_lidar is not None:
            # When the data is already loaded.
            return self._T_radar_ars548_lidar
        else:
            # Calculate data if it is calculated yet.
            self._T_radar_ars548_lidar = np.dot(self.t_radar_ars548_camera, self.t_camera_lidar)
            return self._T_radar_ars548_lidar
...
This method returns the corresponding intrinsic and extrinsic transformation from the dataset.
        ...
        if sensor == 'lidar':
            try:
                calibration_file = os.path.join(self.frame_data_loader.kitti_locations.calib_dir,
                                                f'{self.frame_data_loader.frame_number}.txt')
            except FileNotFoundError:
                logging.error(f"{self.frame_data_loader.frame_number}.txt does not exist at"
                              f" location: {self.frame_data_loader.kitti_locations.calib_dir}!")
                return None, None
        else:
